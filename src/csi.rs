...
impl VolumeRegistry {
    async fn create(&self, name: &str, capacity: u64) -> String {
    ...
    async fn delete(&self, volume_id: &str) {
        let mut vols = self.volumes.lock().await;
...
impl HfCsiController {
    ...
    /// Validate that a repo or bucket exists on HF Hub.
...
impl HfCsiNode {
    ...
    async fn publish_unix(
        ...
        let mut cmd = tokio::process::Command::new("hf-mount-nfs");
        cmd.arg(source).arg(target_path).arg("--nfs-bind").arg("127.0.0.1:0");
        if read_only {
            cmd.arg("--read-only");
        }
        ...
        let child = cmd
            .spawn()
            .map_err(|e| Status::internal(format!("spawn hf-mount-nfs: {e}")))?;
    ...
    async fn publish_windows(
        ...
        let mut cmd = tokio::process::Command::new("hf-mount-nfs.exe");
        cmd.arg(source).arg(target_path).arg("--nfs-bind").arg("0.0.0.0:2049");
        if read_only {
            cmd.arg("--read-only");
        }
        ...
        let mount_result = tokio::process::Command::new("mount")
            .args(["-o", "anon", r"\\127.0.0.1\", target_path])
            .output()
            .await;
