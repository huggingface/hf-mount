...
fn main() {
    ...
    match cli.command {
        Command::Stop { mount_point } => stop(mount_point),
        Command::Status => status(),
        Command::Start { fuse, args } => start(fuse, args),
    }
}

#[cfg(unix)]
fn stop(mount_point: PathBuf) {
    match hf_mount::daemon::stop_daemon(&mount_point) {
        Ok(()) => {}
        Err(e) => {
            eprintln!("Error: {e}");
            std::process::exit(1);
...
#[cfg(not(unix))]
fn stop(_mount_point: PathBuf) {
    eprintln!("Error: daemon stop is not supported on Windows");
    std::process::exit(1);
}
...
#[cfg(unix)]
fn start(fuse: bool, args: Vec<String>) {
    ...
    let mount_point = match mount_point {
        ...
        None => {
            ...
            eprintln!("Usage: hf-mount start [--fuse] [backend-options...] <subcommand> <source> <mount-point>");
            std::process::exit(1);
        }
    };
...
fn start(fuse: bool, args: Vec<String>) {
    ...
    let status = match cmd.status() {
        Ok(s) => s,
        Err(e) => {
            eprintln!("Error: failed to run {}: {e}", backend_path.display());
            std::process::exit(1);
        }
    };
